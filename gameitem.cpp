...
void GameItem::still()
{
    ...
    g_body->SetAngularDamping(0.0);
    g_body->SetLinearVelocity(b2Vec2 (0, 0));
}

void GameItem::setBirdStart()
{
    still();
    g_body->SetTransform(b2Vec2(5.0,8.0),0);
}

void GameItem::setPos(float posX, float posY)
{
    still();
    g_body->SetTransform(b2Vec2(posX,posY),0);
}

void GameItem::moveOutside()
{
    still();
    g_body->SetTransform(b2Vec2(-1.0,0),0);
}

float GameItem::getNowX()
{
    return g_body->GetPosition().x;
}

float GameItem::getNowY()
{
    return g_body->GetPosition().y;
}

void GameItem::accelerateBird4Top()
{
    g_body->SetLinearVelocity(b2Vec2 (40, 10));
}

void GameItem::accelerateBird4Down()
{
    g_body->SetLinearVelocity(b2Vec2 (40, -10));
}

float GameItem::getVelocityX()
{
    return g_body->GetLinearVelocity().x;
}

float GameItem::getVelocityY()
{
